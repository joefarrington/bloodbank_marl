...
def run_scenario_analysis(additional_args: List) -> None:
    for issuing_policy in heuristic_issuing_policy_targets:
        for a in additional_args:
            args = [
                ...
                "+experiment=simple_two_product_perishable/base_case_simple_ga_fit_rep_for_input_comparisons_heuristic_issuing",
            ] + a
    ...
    for a in additional_args:
        args = [
            "+experiment=simple_two_product_perishable/base_case_simple_ga_fit_both_for_input_comparisons",
        ] + a
        command = ["python", "run_evosax.py"] + args
        subprocess.call(command)
