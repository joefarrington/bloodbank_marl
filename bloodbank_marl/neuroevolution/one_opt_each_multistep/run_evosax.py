...
def main(cfg):
    ...
    issue_strategy = hydra.utils.instantiate(
        cfg.evosax.issuing.strategy, num_dims=issue_param_reshaper.total_params
    )
    ...
    # TODO: "Generations" not strictly true in this implementation for the outer loop
    for gen in range(cfg.evosax.num_generations):
        ...
        for i in range(cfg.evosax.n_rep_updates):
            rep_x, rep_state = rep_strategy.ask(rng_ask_rep, rep_state)
            issue_x, _ = issue_strategy.ask(rng_ask_issue, issue_state)
            x = jnp.hstack([rep_x, issue_x])
            reshaped_params = train_param_reshaper.reshape(x)
            fitness = train_evaluator.rollout(rng_train, reshaped_params)[0].mean(
                axis=-1
            )
        ...
        for i in range(cfg.evosax.n_issue_updates):
            rep_x, _ = rep_strategy.ask(rng_ask_rep, rep_state)
    ...
    policy_rep = hydra.utils.instantiate(cfg.policies.replenishment)
    policy_issue = hydra.utils.instantiate(cfg.policies.issuing)
    policy_params = test_param_reshaper.reshape(jnp.array([mean_params]))
